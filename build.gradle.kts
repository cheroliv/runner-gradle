...
tasks.register("info") {
    ...
    doLast {
        ...
        println("  collectCompositeContext  Pipeline N0→N3 complet (graphify + codebase + codex + training + capsule)")
...
tasks.register("collectCompositeContext") {
    ...
    val outputDir = layout.buildDirectory.dir("runner")
    ...
    doLast {
        ...
        if (!skipGraphify && graphifyDir.resolve("build.gradle.kts").exists()) {
            println("[runner] collectFromWorkspace (N0 — graphify) : scanning workspace → graph.json")
            ...
            if (gfExit == 0 && graphFile.exists()) {
                ...
                if (graph is Map<*, *>) {
                    ...
                    println("[runner] OK collectFromWorkspace OK — $nodeCount nodes, $edgeCount edges")
                ...
                println("[runner] collectFromWorkspace : échec (exit=$gfExit)")
            }
        } else if (skipGraphify) {
            println("[runner] collectFromWorkspace : skipped (--skipGraphify)")
        }
        ...
        if (contextMode in listOf("composite", "rag")) {
            if (codexDir.resolve("doc-pipeline").exists()) {
                ...
                println("[runner] codexRetrieve : \"${query.take(80)}\" -> pgvector (top-$topK)")
                ...
                if (codexExit == 0 && retrieveJson.exists()) {
                    ...
                    println("[runner] OK codexRetrieve OK - ${codexEntries.size} results")
                } else {
                    println("[runner] codexRetrieve: pgvector not available (exit=$codexExit)")
                }
            } else {
                println("[runner] codex-gradle (N2) not found - skipping codexRetrieve")
            }
        }
        ...
        if (contextMode in listOf("composite", "rag")) {
            ...
            if (trainingPluginDir.exists()) {
                ...
                println("[runner] collectFromCorpus : \"${query.take(80)}\" -> pgvector (top-$topK)")
                ...
                if (trainExit == 0 && retrieveJson.exists()) {
                    ...
                    println("[runner] OK collectFromCorpus OK - ${trainingEntries.size} results")
                } else {
                    println("[runner] collectFromCorpus: pgvector not available (exit=$trainExit)")
                }
            } else {
                println("[runner] training-gradle (N2) not found - skipping collectFromCorpus")
            }
        }
        ...
        if (contextMode in listOf("composite")) {
            ...
            if (capsulePluginDir.exists()) {
                println("[runner] capsuleCompositeContext")
                ...
                if (capExit == 0 && capsuleJsonFile.exists()) {
                    ...
                    println("[runner] OK capsuleCompositeContext OK - ${capsuleEntries.size} decks")
                } else {
                    println("[runner] capsuleCompositeContext: capsule not available (exit=$capExit)")
                }
            } else {
                println("[runner] capsule-gradle (N2) not found - skipping capsuleCompositeContext")
            }
        }
        ...
        if (contextMode in listOf("composite", "rag")) {
            ...
            if (codebasePluginDir.exists()) {
                println("[runner] collectFromCodebase : pgvector indexing + composite context (EAGER+RAG+Graphify)")
                ...
                if (cbExit == 0) {
                    ...
                    if (workspaceContextFile.exists()) {
                        ...
                        println("[runner] OK collectFromCodebase OK — ${workspaceContextFile.length()} bytes, ${ctxContent.lines().size} lines")
                    } else {
                        println("[runner] collectFromCodebase : OK mais workspace-context.txt non trouvé")
                    }
                } else {
                    println("[runner] collectFromCodebase : pgvector not available (exit=$cbExit)")
                }
            } else {
                // Fallback : codebase-plugin/ absent → utiliser codebase-gradle/ racine
                val buildFile = codebaseDir.resolve("build.gradle.kts")
                if (buildFile.exists()) {
                    println("[runner] collectFromCodebase : codebase-plugin/ absent, tentative racine codebase-gradle/")
                    ...
                    if (cbExit == 0) {
                        ...
                        println("[runner] OK collectFromCodebase OK (fallback)")
                    } else {
                        println("[runner] collectFromCodebase : échec fallback (exit=$cbExit)")
                    }
                } else {
                    println("[runner] codebase-gradle (N1) not found — skipping composite context")
                }
            }
        }
        ...
        println("[runner] OK collectCompositeContext — N0:graphify(${graphifyEntries.size}) N1:codebase(${codebaseEntries.size}) + N2:codex(${codexEntries.size})/training(${trainingEntries.size})/capsule(${capsuleEntries.size}) → ${compositeOutput.absolutePath}")
    }
}
...
val boroughs = listOf(
    ...
    BoroughDef("New Jersey",    "$workspaceDir/foundry/public/runner-gradle",                      3, "N3",   "orchestrateur",  false, null,                          null,                         null,       null),
